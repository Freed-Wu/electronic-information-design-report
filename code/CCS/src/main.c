   ...
   while(key1)
	...
	{	
		...
		while(key5)
		{
     	...
     	EvaRegs.T1CON.bit.TPS=2;//change top
		adfreq_last=1000*adfreq[0]+100*adfreq[1]+10*adfreq[2]+adfreq[3];	
		EvaRegs.T1PR=18750/adfreq_last*1000-1;
		adfreq[0]=0;
		adfreq[1]=0;
		adfreq[2]=0;
        adfreq[3]=0;//change end
...
void input_display()//change top
{	
		int copy_x=0;
		int copy_count=0;
		adfreq[count_ad]=x;       //count_ad:ÊýÎ»  copy_count:¿ØÖÆÏÔÊ¾ÔÚÄÇÒ»ÁÐ  copy_x:¹â±ê
		if(count_ad==3)
		{
			copy_x=adfreq[0];
			copy_count=1;
		...
		count_ad=count_ad+1;
		wr_hex(x,0x010,0x05+15-count_ad*3,FALSE);
	  //wr_hex(x,0x010,0x05+12,FALSE);
		wr_hex(copy_x,0x010,0x05+15-copy_count*3,1);
		if(count_ad>=4)count_ad=0;
}//change end
...
{	
		...
		count_ad=count_ad+1;
		wr_hex(x,0x010,0x05+count_ad*3,FALSE);
//		wr_hex(31,0x010,0x05+12,FALSE);
		wr_hex(copy_x,0x010,0x05+copy_count*3,1);
		if(count_ad>=4)count_ad=0;
}*/
...
/*			  FALSE on failure													  */
/*Write_SX2reg°ÑvalueÐ´µ½µØÖ·Îª addr µÄ¼Ä´æÆ÷*/
/**********************************************************************************/
BOOL Write_SX2reg(unsigned char addr, unsigned int value)
{
...
/*void output_dispaly()
{
  ...
  { 
    ...
    wr_hex(height,0x00+i,0x01+height,FALSE);
  }
}  //change end   
